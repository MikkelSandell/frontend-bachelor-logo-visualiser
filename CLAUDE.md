...
| `apps/admin/src/api/productApi.ts` | All Admin → backend API calls. Includes `ensureToken()` which fetches a dev JWT on first write. All zone changes are batched and sent via `updateProduct()` (PUT with full product + zones list). `normalizeProduct()` normalises API responses, including mapping `allowedTechniques` from backend `{id, name}` objects to plain `PrintTechnique` strings. Also exports `uploadFixedLogo()` — public (no auth) upload of a fixed logo image. `ZoneUpsertPayload` includes `imageUrl` so the per-zone side image is preserved on save. |
